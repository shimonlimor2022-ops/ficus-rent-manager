...
export default async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    return json({ members, isOwner: user.role === 'owner' });
  }
    ...
    if (process.env.RESEND_API_KEY && FROM_EMAIL) {
      ...
      try {
        await resend.emails.send({
          from: FROM_EMAIL,
          to: [cleanEmail],
          subject: 'You’ve been invited to Ficus Investments',
          html: `<div style="font-family:Arial,sans-serif;font-size:15px;line-height:1.6;color:#111008">
            <p>${user.email} has invited you to join the Ficus Investments rent management team.</p>
            <p><a href="${joinUrl}" style="color:#111008;font-weight:700;">Click here to join the team</a> (link expires in 7 days).</p>
          </div>`,
        });
      } catch (err) {
