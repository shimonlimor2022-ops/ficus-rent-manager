...
// Returns the logged-in team_user row (or null) based on the session cookie.
export async function getSessionUser(req) {
  const token = getCookie(req, 'session');
  if (!token) return null;
  const [row] = await sql`SELECT * FROM team_user WHERE session_token = ${token}`;
  return row || null;
}

// Returns true if the request has a valid session for any team member.
